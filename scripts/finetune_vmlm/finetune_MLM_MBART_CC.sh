...
EXP_NAME="MBART_MLM_en-fr_CC_from_NMT_adapters_textparamsfrozen_PMask50"
EPOCH_SIZE=2000000
seed=12345
...
main.py --beam_size 4 --exp_name ${EXP_NAME} --dump_path ${DUMP_PATH} \
  ...
  --stopping_criterion 'valid_en_mlm_acc,20' --validation_metrics 'valid_en_mlm_acc' \
  --iter_seed ${seed} --other_seed ${seed} --smoothing 0.1 --save_periodic 1 --model_type mbart \
  --cache_dir ${CACHE_HUGGINGFACE} --amp 1 --fp16 True --accumulate_gradients 4 \
  --reload_model /gpfsstore/rech/ncm/ueh14bh/guided_sa/en-fr/MBART_nmt_en-fr_OPUS/2006603/best-valid_en-fr_mt_bleu.pth \
  --adapters --freeze_text_parameters --prob_mask_text 0.5 --start_new_xp_from_ckpt $@
