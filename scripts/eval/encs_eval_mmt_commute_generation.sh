...
WANDB_MODE=offline python main.py --beam_size 8 --dump_path ${DUMP_PATH} --features_path ${FEAT_PATH} --features_type 'mdetr+clip' \
  --data_path ${DATA_PATH} --src_lang 'en' --tgt_lang 'cs' --dropout '0.4' --batch_size 1 --optimizer 'adam_inverse_sqrt,lr=0.0001,warmup_updates=2000' \
  --max_epoch 500 --max_len 80 --num_workers 4 --stopping_criterion 'valid_en-cs_mt_bleu,50' --validation_metrics 'valid_en-cs_mt_bleu' \
  ...
  --eval_only true --multimodal_model --encoder_attn_mask_text_only --adapters --test_data_set commute --guided_self_attention --commute_generation --eval_bleu true $@
