...
EXP_NAME="MBART_mix_MMT_VMLM_en-cs_Multi30k_from_NMT_adapters_guidedSA_textparamsfrozen_CLIPMDETR_mix50_PMask25_lr1e5_500_EPOCH_TRAINING_100minEpochs"
EPOCH_SIZE=58000
seed=42
...
main.py --beam_size 4 --exp_name ${EXP_NAME} --dump_path ${DUMP_PATH} --features_path ${FEAT_PATH}  --features_type 'mdetr+clip' \
  --data_path ${DATA_PATH} --data_mix_path ${DATA_MIX_PATH} --features_mix_path ${FEAT_PATH_MIX} --src_lang 'en' --tgt_lang 'cs' --dropout '0.4' \
  --batch_size 16 --optimizer 'adam,lr=0.00001' --epoch_size ${EPOCH_SIZE} --eval_bleu true --max_epoch 5000 --max_len 80 --num_workers 0 \
  --stopping_criterion 'valid_en-cs_mt_bleu,10' --validation_metrics 'valid_en-cs_mt_bleu' \
