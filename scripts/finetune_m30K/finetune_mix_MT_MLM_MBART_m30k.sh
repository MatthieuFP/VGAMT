...
EXP_NAME="MBART_mix_MT_MLM_en-fr_Multi30k_from_NMT_adapters_textparamsfrozen_mix50_PMask25"
EPOCH_SIZE=58000
seed=42
...
main.py --beam_size 4 --exp_name ${EXP_NAME} --dump_path ${DUMP_PATH} --data_path ${DATA_PATH} --data_mix_path ${DATA_MIX_PATH} \
  --src_lang 'en' --tgt_lang 'fr' --dropout '0.4' --batch_size 16 --optimizer 'adam,lr=0.0001' --epoch_size ${EPOCH_SIZE} \
  --eval_bleu true --max_epoch 5000 --max_len 80 --num_workers 0 --stopping_criterion 'valid_en-fr_mt_bleu,20' \
  --validation_metrics 'valid_en-fr_mt_bleu' --iter_seed ${seed} --other_seed ${seed} --smoothing 0.1 --save_periodic 1 \
  ...
  --adapters --freeze_text_parameters --start_new_xp_from_ckpt --mix_xp .5 --prob_mask_text 0.25 --encdec_vmlm $@
