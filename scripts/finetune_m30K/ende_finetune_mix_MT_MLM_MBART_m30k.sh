...
EXP_NAME="MBART_mix_MT_MLM_en-de_Multi30k_from_NMT_adapters_textparamsfrozen_mix50_PMask25_lr1e4_smaller_batch_100minEPOCHS"
EPOCH_SIZE=58000
seed=42
...
main.py --beam_size 4 --exp_name ${EXP_NAME} --dump_path ${DUMP_PATH} --data_path ${DATA_PATH} --data_mix_path ${DATA_MIX_PATH} \
  --src_lang 'en' --tgt_lang 'de' --dropout '0.4' --batch_size 16 --optimizer 'adam,lr=0.0001' --epoch_size ${EPOCH_SIZE} \
  --eval_bleu true --max_epoch 5000 --max_len 80 --num_workers 0 --stopping_criterion 'valid_en-de_mt_bleu,10' \
  --validation_metrics 'valid_en-de_mt_bleu' --iter_seed ${seed} --other_seed ${seed} --smoothing 0.1 --save_periodic 1 \
